...
/**
	...
	public static boolean isNotNull(String str){
		return isNotBlank(str)&&!str.equals("null");
	}

	/**
	 * 判断obj是不是null，是返回“”，否返回tostring
	 * 
	 * @param obj
	 * @return
	 */
	...
	public static boolean hasChinese(String str) {
		return hasExpression(str, "[\u4e00-\u9fa5]");
	}

	// 匹配双字节字符
	public static boolean hasDoubleByte(String str) {
		return hasExpression(str, "[^\\x00-\\xff]");
	}
	...
	public static String pathReplace(String s) {
		return s.replaceAll("//", CommonConst.SEP);
	}
	...
	public static void deleteLast(StringBuffer hql) {
		int lengh = hql.length();
		if (lengh > 4) {
			if (hql.substring(lengh - 4, lengh).equals(" and")) {
				hql.delete(lengh - 4, lengh);
			}
		}
		lengh = hql.length();
		if (lengh > 6) {
			if (hql.substring(lengh - 6, lengh).equals(" where")) {
				hql.delete(lengh - 6, lengh);
			}
		}
	}
	...
	public static void main(String[] args) {
		System.out.println(StringUtil.getTicket("WP"));
	}
	
//	public static String getTicket(){
//		Date d = new Date();
//		String s = d.getTime()+getRndString(4);
//		return s;
//	}
//	
	public static String getTicket(String prodCategory){
		Date d = new Date();
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss"); 
		String s = sdf.format(d)+prodCategory+getRndString(4);
		return s;
	}
	
	public static ConcurrentHashMap<String, String> json2map(String jsonStr){
		ConcurrentHashMap<String, String> temp_map = new ConcurrentHashMap<String, String>();
		try {
			String[] items = jsonStr.split("&");
			for(int i=0; i<items.length; i++){
				temp_map.put(items[i].split("=")[0], items[i].split("=").length==2?URLDecoder.decode(items[i].split("=")[1], "UTF-8"):"");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static List<String> json2list(String jsonStr){
		...
		try {
			String[] items = jsonStr.split("&");
			for(int i=0; i<items.length; i++){
				temp_list.add(URLDecoder.decode(items[i].split("=")[1], "UTF-8"));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public static String creatNginxConf(String domain, String username) {
		String nginx=" server{ listen 80; server_name " +
				domain+"; location / { " +
				" index  index.html index.htm index.jsp; proxy_pass http://localhost:" +
					ConfConst.TOMCAT_PORT+";" +
				" proxy_set_header Host $host;  proxy_set_header X-Real-IP $remote_addr; " +
				"rewrite ^/$ /vip/"+username+"/index last; rewrite ^/(?<site>\\w*)$ " +
				"/vip/"+username+"/$site last;  }   }";
//		String nginx="server { listen 80; server_name "+domain+";  rewrite ^/(?<aa>.*)$  http://"+username+"."+ConfConst.URL+"/$aa permanent; }  ";
			return nginx;
		}
}
