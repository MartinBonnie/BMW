...
public class RegexUtil {
	...
	public static boolean checkPassword(String password) {
		// http://www.163.com
		String reg = CommonConst.REGEX_PASSWORD;
	 ...
	 * 检验用户名 取值范围为a-z,A-Z,0-9,"_",汉字，不能以"_"结尾 用户名有最小长度和最大长度限制，比如用户名必须是4-20位
	 */
	public static boolean checkUsernameNotCh(String username, int min, int max){
			String regex = "[\\w]{" + min + "," + max + "}(?<!_)";
			return startCheck(regex, username);
	}
	
	public static void main(String[] args){
		System.out.println(RegexUtil.checkUsernameNotCh("1233f456d_d", 0, 20));
	}
}