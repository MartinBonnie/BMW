...
//			}
		...
		for(var j=Math.round(parseFloat(tableData.bottomValue)/5)*5; j<=tableData.topValue+loopInterval; j+=loopInterval){
			yLabels.push(j); 
		}
		if(yLabels[yLabels.length-1] > tableData.topValue) {
			tableData.topValue = yLabels[yLabels.length-1];
			totalYRange = tableData.totalYRange = tableData.topValue - tableData.bottomValue;
		} else {
					...
					for(var h=0; h<dataGroups.length; h++){
						...
						for(var i=0; i<points.length; i++){
							...
							if (points[i].value != 0) {
								...
								ctx.moveTo(xVal, 0);
								var yVal = Math.round(-points[i].value*yScale);
								ctx.lineTo(xVal, yVal);
								ctx.fillText(points[i].value, xVal, yVal-5);
	                        }
							integer+=barWidth;
						}
		...
		if(o.interaction) {
			...
			tracker.mousemove(function(e){
				...
				$.each(charts[o.type].interactionPoints,function(i,current){
					...
					if(dist < minDist && dist <= o.multiHover) {
						found = [current];
						minDist = dist;
					}
				});
