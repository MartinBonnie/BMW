			...
			<div id="main-content"> <!-- Main Content Section with everything --> 
				...
				<div class="block">
					...
					<div class="block_content">
						...
						<form action="/web/member/news/saveEdit" method="post">
						<div class="form">
							<p>
								<label>标题:</label><br />
								<input type="hidden" name="newsId" id="newsId" value="${news.newsId}"/>
								<input name="newsTitle" type="text" class="text small" value="${news.newsTitle}"/> 
							...
							<p>
								<label>系统分类:</label> <br /> <select class="styled"
									...
									<c:forEach items="${newsCategories}" var="newsCategory">
												<option value="${newsCategory.sysId}"  <c:if test="${newsCategory.sysId==news.newsCategory}">selected</c:if>>${newsCategory.sysName}</option>
									</c:forEach>
								</select>
							</p>
							<p>
									...
									<c:forEach items="${categories}" var="category">
										<c:choose>
											<c:when test="${category.getSubCategory().size()==0}">
												<option value="${category.categoryId}" <c:if test="${category.categoryId==news.userCategory}">selected</c:if>>${category.name}</option>
											</c:when>
											<c:otherwise>
												<optgroup label="${category.name}">
													<c:forEach items="${category.getSubCategory()}" var="subCategory">
														<option value="${subCategory.categoryId}" <c:if test="${category.categoryId==news.userCategory}">selected</c:if>>${subCategory.name}</option>
													</c:forEach>
												</optgroup>
											</c:otherwise>
										</c:choose>
							...
							<p>
								<label>关键字:</label><br />
								<input name="newsTag" type="text" class="text medium error" value="${news.newsTag}"/> 
								<span class="note">多个关键字请用空格隔开</span>
							</p>
							<p>
								<label>产品简介:</label><br />
								<textarea id="newsContent" name="newsContent" class="wysiwyg">${news.newsContent}</textarea>
							</p>
							<!-- 
							<p class="fileupload">
								<label>新闻图片:</label><br />
